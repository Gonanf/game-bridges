# PokeAI – PyBoy (GB/GBC) + PyGBA/mgba (GBA) + 3 agentes Planning/Execution/Critique via Kateto
...
async def run_pyboy(rom_path):
    """Run Pokémon GB/GBC via PyBoy"""
    from pyboy import PyBoy
    print(f"PyBoy ROM {rom_path} iniciando...")
    pyboy = PyBoy(str(rom_path), window="null")
    
    await post("planning", "Modo Juego: pokeai – Pokémon GB agent activado.", rms=0.2, event_type="game_start", 
    ...
    while step < max_steps:
        step += 1
        game_state = {
            "step": step, "position": {"map": "Pallet Town", "x": step % 10, "y": (step*3) % 10},
            "party": [{"name": "PIKACHU", "level": 5+step//5, "hp": 20-step%10}],
            "badges": min(step//10, 8), "pokedex_count": step,
            "in_battle": step % 5 == 0,
            "inventory": {"pokeball": 10-step//10, "potion": 5}
        }
        await post("planning", f"Paso {step}: ¿Qué debería hacer en {game_state['position']}?", rms=0.25, event_type="state_update", state=game_state)
...
async def run_pygba(rom_path):
    """Run Pokémon GBA via PyGBA/mgba"""
    try:
        from pygba import PyGBA
        import mgba.core
        print(f"PyGBA ROM {rom_path} iniciando...")
        core = mgba.core.load_path(str(rom_path))
        if core is None:
            raise RuntimeError(f"Failed to load GBA ROM: {rom_path}")
        core.reset()
        print("GBA core ready")
    except Exception as e:
        print(f"PyGBA init failed: {e}, fallback mock")
        await mock_loop()
        return
    
    await post("planning", "Modo Juego: pokeai – Pokémon GBA agent activado.", rms=0.2, event_type="game_start", 
               state={"game":"pokeai","profile":"pokeai","system_prompt":"Eres un agente Pokémon (GBA: FireRed/Emerald). 3 voces: Jane (planning), Doktor (execution), Conquest (critique). Herramientas: mempalace_query/mempalace_append/web_search. Objetivo: explorar, capturar, entrenar, ganar el juego."})
    ...
    while step < max_steps:
        step += 1
        game_state = {
            "step": step, "position": {"map": "Kanto", "x": step % 10, "y": (step*3) % 10},
            "party": [{"name": "CHARIZARD", "level": 36+step//3, "hp": 100-step%20}],
            "badges": min(step//8, 8), "pokedex_count": step*2,
            "in_battle": step % 6 == 0,
            "inventory": {"pokeball": 15-step//8, "potion": 8, "super_potion": 3}
        }
        await post("planning", f"Paso {step}: ¿Qué debería hacer en {game_state['position']}?", rms=0.25, event_type="state_update", state=game_state)
        await asyncio.sleep(0.3)
        try:
            async with httpx.AsyncClient(timeout=5) as c:
                ...
                if r.status_code == 200:
                    ...
                    if last_action:
                        print(f"[bridge action] {last_action}")
                        # mgba/PyGBA: run frame to advance
                        core.run_frame()
                        await post("execution", f"Avanzado 1 frame (bridge: {last_action})", rms=0.2)
        except:
            core.run_frame()
            await post("execution", f"Avanzado 1 frame (sin bridge)", rms=0.2)
        await asyncio.sleep(0.3)
        await post("critique", f"Crítica paso {step}: {random.choice(['OK','ajustar','reintentar'])}", rms=0.22)
        await asyncio.sleep(0.5)
    await post("planning", f"PokeAI GBA: sesión real completada ({step} pasos).", rms=0.3, event_type="game_end")

async def real_loop(rom_path):
    ext = pathlib.Path(rom_path).suffix.lower()
    if ext in (".gb", ".gbc"):
        await run_pyboy(rom_path)
    elif ext == ".gba":
        await run_pygba(rom_path)
    else:
        print(f"Formato no soportado: {ext}, usando mock")
        await mock_loop()
